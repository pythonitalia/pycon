...
- Of the 8 soft columns, exactly **4** lack `blank=True` today: `why`, `python_usage`, `been_to_other_events`, `occupation`. The other 4 (`age_group`, `community_contribution`, `gender`, `notes`) are already `blank=True`. All 8 are NOT NULL at the DB level (`blank=True` is Python-only) — `None` must never reach `Grant.objects.create`.
