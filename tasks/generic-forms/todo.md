...
- [ ] **T5.2** New-submission integration: fetch `form(GRANT)`, swap the 6 dynamic inputs for DynamicForm (gender/occupation selects stay), `answers` in payload (drop the 6 legacy dynamic fields), **null-form guard blocks submission**, strip legacy validation selections from submit-grant.graphql, prune dead options.ts constants. Verify: pnpm test/build + manual submit.
