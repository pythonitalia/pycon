...
def test_submit_vote(
    ...
    conference = ConferenceFactory(active_voting=True)
...
def test_reject_vote_when_voting_is_not_open(graphql_client, user, requests_mock):
    graphql_client.force_login(user)
...
def test_reject_vote_when_submission_is_invalid(graphql_client, user, requests_mock):
    graphql_client.force_login(user)
...
def test_user_can_vote_different_submissions(graphql_client, user, requests_mock):
    graphql_client.force_login(user)
    ...
    submission1 = SubmissionFactory(conference=conference)
    ...
    submission2 = SubmissionFactory(conference=conference)
...
def test_updating_vote_when_user_votes_the_same_submission(
    graphql_client, user, requests_mock
):
    graphql_client.force_login(user)
    ...
    submission = SubmissionFactory(conference=conference, id=1)
...
def test_cannot_vote_without_a_ticket_or_membership(
    ...
    graphql_client.force_login(user)
    submission = SubmissionFactory(conference__active_voting=True)
...
@mark.django_db
def test_only_authenticated_users_can_vote(graphql_client):
