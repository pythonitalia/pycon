...
def migrate_grants(apps, schema_editor):
    ...
    for grant in grants:
        try:
            ...
            if not categories:
                logger.warning(
                    f"No reimbursement categories found for conference {grant.conference_id}. "
                    f"Skipping grant {grant.id}."
                )
                skipped_count += 1
                continue

            def add_reimbursement(category_key, amount):
                """Helper to add reimbursement if category exists and amount is valid."""
                if category_key not in categories:
                    logger.warning(
                        f"Category '{category_key}' not found for grant {grant.id}. Skipping."
                    )
                    return False
                if not amount or amount == 0:
                    logger.debug(
                        f"Amount is None or 0 for grant {grant.id}, category '{category_key}'. Skipping."
                    )
                    return False
            ...
            logger.error(f"Error migrating grant {grant.id}: {e}")
            error_count += 1

    logger.info(
        f"Migration completed: {migrated_count} grants migrated, "
    ...
    operations = [
        ...
        migrations.CreateModel(
            ...
            fields=[
                ...
                ('max_amount', models.DecimalField(decimal_places=0, default=Decimal('0.00'), help_text='Maximum amount for this category', max_digits=6)),
        ...
        # Ensure categories exist before migrating grants
