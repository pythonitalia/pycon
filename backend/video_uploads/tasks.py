...
def process_wetransfer_to_s3_transfer_request(request_id):
    with transaction.atomic():
        wetransfer_to_s3_transfer_request = (
            VideosImportRequest.objects.select_for_update().get(id=request_id)
        )

        if (
            wetransfer_to_s3_transfer_request.status
            != VideosImportRequest.Status.QUEUED
        ):
            logger.warning(
        ...
        wetransfer_to_s3_transfer_request.status = VideosImportRequest.Status.PROCESSING
    ...
    wetransfer_to_s3_transfer_request = VideosImportRequest.objects.get(id=request_id)

    try:
        processing = WetransferProcessing(wetransfer_to_s3_transfer_request)
        imported_files = processing.run()

        wetransfer_to_s3_transfer_request.status = VideosImportRequest.Status.DONE
        ...
        wetransfer_to_s3_transfer_request.status = VideosImportRequest.Status.FAILED
