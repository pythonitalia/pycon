...
def test_get_voucher(requests_mock, pretix_voucher_data):
...
def test_get_voucher_with_no_quota_and_item_id_is_marked_as_all_items(
    pretix_voucher_data, requests_mock
):
...
@override_settings(PRETIX_API="https://pretix/api/")
def test_get_voucher_with_quota_items(pretix_voucher_data, requests_mock):
    pretix_voucher_data["item"] = None
...
@override_settings(PRETIX_API="https://pretix/api/")
def test_get_voucher_with_invalid_code(requests_mock):
    conference = ConferenceFactory()
    requests_mock.get(
        "https://pretix/api/organizers/base-pretix-organizer-id/events/base-pretix-event-id/extended-vouchers/TRYR6CWFKQHL2WGN/",
        status_code=404,
