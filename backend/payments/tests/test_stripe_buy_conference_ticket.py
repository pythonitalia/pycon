...
@mark.django_db
def test_success_order_correctly_generates_a_client_secret(
    ...
    with patch("payments.providers.stripe.stripe.PaymentIntent.create") as create:
        create_mock = Mock()
        create_mock.id = "123456789"
        create_mock.client_secret = "abc"
        create.return_value = create_mock
    ...
    assert response["data"]["buyTicketWithStripe"]["clientSecret"] == "abc"
...
@mark.django_db
@mark.integration
def test_stripe_integration_buying_conference_ticket(
    graphql_client, user, ticket_fare_factory
):
    graphql_client.force_login(user)
