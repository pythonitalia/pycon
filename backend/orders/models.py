...
from django_fsm import RETURN_VALUE, FSMIntegerField, transition
...
class Order(TimeStampedModel):
    ...
    transaction_id = models.TextField(_("transaction id"), blank=True, default="")

    state = FSMIntegerField(default=PaymentState.NEW, choices=PaymentState.choices())
    ...
    def charge(self, payload={}):
        ...
        if not provider_class:
            raise ValueError(
                _("Provider %(provider)s not known!") % {"provider": self.provider}
            )
    ...
    @transition(
        ...
        target=RETURN_VALUE(PaymentState.COMPLETE),
        on_error=PaymentState.FAILED,
    )
    def fullfil(self):
        """
    ...
    @transition(
        field=state,
        source="*",
        target=RETURN_VALUE(PaymentState.FAILED),
        on_error=PaymentState.FAILED,
    )
    def fail(self):
        """
...
class OrderItem(TimeStampedModel):
    ...
    item_type = models.ForeignKey(
        ContentType, on_delete=models.CASCADE, verbose_name=_("item type")
    )
    item_object_id = models.PositiveIntegerField(_("item object id"))
    item_object = GenericForeignKey("item_type", "item_object_id")

    def fullfil(self):
        for __ in range(0, self.quantity):
            self.item_object.fullfil(order=self.order, user=self.order.user)
