...
from association.tests.api import graphql_client
...
async def _(graphql_client=graphql_client, db=db):
    ...
    with patch(
        "customers.domain.repository.stripe.checkout.Session.create",
    ) as mock_create_session, patch(
        "customers.domain.repository.stripe.Customer.list"
    ) as mock_customers_list, patch(
        "customers.domain.repository.stripe.Customer.create"
